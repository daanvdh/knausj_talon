...
class Actions:
    ...
    def before(symbol: str, occurrence_number: int):
        """go right until you find the given symbol for the occurrence_number-th time and put the cursor before it"""
        before(ascii(symbol), int(occurrence_number))
        
    def after(symbol: str, occurrence_number: int):
    ...
    def backwards_before(symbol: str, occurrence_number: int):
        """go left until you find the given symbol for the occurrence_number-th time and put the cursor before it"""
        backwards_before(ascii(symbol), int(occurrence_number))
        
    def backwards_after(symbol: str, occurrence_number: int):
...
def findIndexLeft(symbol, occurrence_number, text, start_index):
    i = start_index
    occurrence_count = 0
    length = len(text) - 1
...
def extentRight(current_selection_length, text, i):
    if i > 0:
        for j in range(0, i):
...
def extentLeft(current_selection_length, text, i):
    if i > 0:
        for j in range(0, i):
...
def before(symbol, occurrence_number):
    text = getTextRight()
    i = findIndexRight(symbol, occurrence_number, text, 0)
    goRight(text, i)
...
def backwards_after(symbol, occurrence_number):
    text = getTextLeft()
    i = findIndexLeft(symbol, occurrence_number, text, 0)
    goLeft(text, i)
    
def backwards_before(symbol, occurrence_number):
    text = getTextLeft()
    i = findIndexLeft(symbol, occurrence_number, text, 0)
    goLeft(text, i+1)

def extend_right_before(symbol, occurrence_number):
    current_selection_length = getCurrentSelectionSize()
    if current_selection_length > 0: 
        actions.edit.left()        
        time.sleep(0.1)
    text = getTextRight()
    i = findIndexRight(symbol, occurrence_number, text, current_selection_length)
    extentRight(current_selection_length, text, i)
...
def extend_right_after(symbol, occurrence_number):
    ...
    text = getTextRight()
    i = findIndexRight(symbol, occurrence_number, text, current_selection_length)
    extentRight(current_selection_length, text, i+1)
...
def extend_left_before(symbol, occurrence_number):
    ...
    if current_selection_length > 0: 
        ...
        time.sleep(0.1)
    text = getTextLeft()
    i = findIndexLeft(symbol, occurrence_number, text, current_selection_length)
    extentLeft(current_selection_length, text, i+1)
...
def extend_left_after(symbol, occurrence_number):
    ...
    text = getTextLeft()
    i = findIndexLeft(symbol, occurrence_number, text, current_selection_length)    
    extentLeft(current_selection_length, text, i)
